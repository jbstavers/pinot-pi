...
Work Units 0–3 provide setup, status, bounded background delegation, the durable Herdr-backed implementer, and the exact test-suite runner. Loading the extension registers commands/tools and performs no file writes. `/pinot-status` reads only Pinot's state metadata and supported Pi location metadata. `/pinot-setup` initializes Pinot's persistent base state; the implementer may create only its named child session directory beneath the setup-created implementer root.
